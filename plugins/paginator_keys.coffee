...
module.exports = (env, callback) ->
  ...
  options = env.config.paginator_keys or {}
  ...
  class PaginatorPage extends env.plugins.Page
    ...
    getView: -> (env, locals, contents, templates, callback) ->
      # simple view to pass articles and pagenum to the paginator_keys template
      # note that this function returns a funciton

      # get the pagination template
      template = templates[options.template]
      if not template?
        return callback new Error "unknown paginator_keys template '#{ options.template }'"
  ...
  # register a generator, 'paginator_keys' here is the content group generated content will belong to
  # i.e. contents._.paginator_keys
  env.registerGenerator 'paginator_keys', (contents, callback) ->

    # find all articles
    articles = getKeys contents
    ...
    rv['index_keys.page'] = pages[0] # alias for first page
    rv['last_keys.page'] = pages[(numPages-1)] # alias for last page

    # callback with the generated contents
    callback null, rv

  # add the article helper to the environment so we can use it later
  env.helpers.getKeys = getKeys
