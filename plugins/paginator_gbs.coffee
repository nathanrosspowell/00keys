...
module.exports = (env, callback) ->
  ...
  options = env.config.paginator_gbs or {}
  ...
  class PaginatorPage extends env.plugins.Page
    ...
    getView: -> (env, locals, contents, templates, callback) ->
      # simple view to pass articles and pagenum to the paginator_gbs template
      # note that this function returns a funciton

      # get the pagination template
      template = templates[options.template]
      if not template?
        return callback new Error "unknown paginator_gbs template '#{ options.template }'"
  ...
  # register a generator, 'paginator_gbs' here is the content group generated content will belong to
  # i.e. contents._.paginator_gbs
  env.registerGenerator 'paginator_gbs', (contents, callback) ->

    # find all articles
    articles = getGBs contents
    ...
    rv['index_gbs.page'] = pages[0] # alias for first page
    rv['last_gbs.page'] = pages[(numPages-1)] # alias for last page

    # callback with the generated contents
    callback null, rv

  # add the article helper to the environment so we can use it later
  env.helpers.getGBs = getGBs
